...
function MajorDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchMajor = async () => {
      ...
      setMajor({ ...data, sections: parsedSections, related_degrees: parsedDegrees });
    };
    fetchMajor();
    return () => { alive = false; };
  }, [id]);
    ...
    const fetchMeta = async () => {
      try {
        if (major.related_degrees?.length > 0) {
          const degreeCodes = Array.from(new Set(major.related_degrees.map((d) => d.degree_code).filter(Boolean)));
        ...
        if (major.sections?.length > 0) {
          ...
          major.sections.forEach((sec) => { (sec.courses || []).forEach((c) => c.code && allCodes.add(c.code)); });
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          <div className="flex-1 min-w-0 space-y-0">
            ...
            {major.sections?.length > 0 && (
              <FlatSection title="Specialisation Structure" icon={<HiBookOpen className="w-4 h-4" />}>
            ...
            {major.related_degrees?.length > 0 && (
              <FlatSection title="Programs Offering This Major" icon={<HiAcademicCap className="w-4 h-4" />}>
          ...
          <div className="w-full lg:w-72 xl:w-80 flex-shrink-0 space-y-4 lg:sticky lg:top-24">
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-5">
              ...
              <div className="space-y-3">
                {major.specialisation_type && (
                  <StatRow icon={<HiCollection className="w-4 h-4 text-sky-600 dark:text-sky-400" />} label="Type" value={major.specialisation_type} />
                )}
                {major.uoc_required && (
                  <StatRow icon={<HiChartBar className="w-4 h-4 text-sky-600 dark:text-sky-400" />} label="UOC Required" value={major.uoc_required} />
                )}
                {major.major_code && (
                  <StatRow icon={<HiDocumentText className="w-4 h-4 text-sky-600 dark:text-sky-400" />} label="Code" value={major.major_code} />
                )}
                {major.faculty && (
                  <div className="pt-2 border-t border-slate-100 dark:border-slate-700">
                    <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Faculty</p>
                    <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed">{major.faculty}</p>
                  </div>
                )}
              </div>
            </div>
